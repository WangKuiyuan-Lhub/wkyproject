...
//sm3_long_to_str 和 sm3_str_to_long：在 unsigned long 和字节数组之间进行转换。
...
//sm3_pad_message：按照SM3算法的要求，对输入消息进行填充，使其长度成为64字节的倍数。
...
//sm3_group_a 和 sm3_group_b：实现SM3算法中的特定步骤。
...
//sm3_str_group：以512位（64字节）的块处理填充后的消息。
...
//sm3_str_summ：执行最终的哈希计算，生成256位（32字节）的哈希值。
